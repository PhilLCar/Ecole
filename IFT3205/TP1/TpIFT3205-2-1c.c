...
int main(int argc,char **argv)
{
  ...
  float r = sqrt(sqr(length / 2) + sqr(width / 2));
  ...
  ConeFilter(imgR, width, length, r, r, Straight, FILTER_IN);
  ConeFilter(imgI, width, length, r, r, Straight, FILTER_IN);
  ...
  printf("\nNuméro 2.1 c) terminé avec succès\n\n");
  return 0; 	 
}
